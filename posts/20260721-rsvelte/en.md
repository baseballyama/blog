...
At [Flyle](https://flyle.io), where I work, we now run multiple AI agents in parallel as a normal way to develop. In this style, agents run type checking and linting after every implementation step, so static checks run far more often than when only humans wrote code. When checks run in parallel, they use up CPU and memory, and each check gets even slower. An agent cannot start its next fix until the check finishes, so **the time a static check takes becomes the lower bound of one loop iteration**.
...
- AI agents run static checks on every loop iteration, so check time becomes the lower bound of iteration time, and CPU/memory usage limits how many agents can run in parallel
- Rust / native tools are making static checks 10x to 100x faster, but the Svelte-specific parts of `.svelte` processing (parsing, transformation, linting, formatting) still run on JavaScript
- rsvelte is a set of Rust implementations that aims to be a drop-in replacement for the Svelte toolchain. It adds nothing to the language semantics. It is early stage, and maturity varies by package
- On Flyle's production frontend (8,795 files checked in the official setup), replacing svelte-check with rsvelte-check alone cut type checking from 51.4s to 30.6s (1.7x, about 56% less CPU). Switching the check engine to tsgo brought it to 9.0s (5.7x combined)
- Svelte-specific linting with an identical rule set is about 20x faster (all 382 diagnostics match between both tools). With 8 checks running at once, the TypeScript work shared by both setups becomes the bottleneck and the gap shrinks. I believe the real value is finishing each check faster, which reduces overlap in the first place

## 1. Why check speed matters now

### 1.1 AI became the implementer, and quality moved to harnesses

When AI agents write most of the code, the way we keep quality changes. Static checks mattered in human-centered development too. But when agents write the code, **harnesses that mechanically verify agent output** become the main way to keep quality.

Harnesses come in several layers: design harnesses that fix the agent's assumptions through documents and types, development harnesses that verify output on every implementation step, and test harnesses that verify behavior. Each is worth its own article, but this one focuses on the core of the development harness: the **static analysis stack** (formatter, type checking, linter, unused-code detection).
...
A bigger machine does help. But if each check process uses the same amount of resources, the cores and memory you need grow with the parallelism. Making the tools more efficient raises the parallelism the same hardware can handle.

## 2. Rust-based toolchains and where Svelte stands

### 2.1 The movement to make static analysis much faster

The movement to rewrite static analysis tools in Rust or native code is already underway.
...
- [Biome](https://biomejs.dev/), a Rust formatter and linter, reaches similar speedups
...
The important point is that when speed changes this much, behavior changes too. Checks that take 10 minutes overlap across parallel agents and fight over resources. Checks that finish in seconds barely overlap at all, so resource contention becomes rare. Better latency also relieves the resource problem.

### 2.2 But the Svelte-specific parts still run on JavaScript
...
- **Type checking**: svelte-check converts `.svelte` files to TypeScript with svelte2tsx and passes them to a check engine. The engine can now be sped up with `--tsgo`, but the transformation and orchestration run at JavaScript speed

In other words, even the official tools can now speed up the TypeScript part with tsgo, but the Svelte-specific work (Svelte parsing, svelte2tsx transformation, template linting, formatting) still runs on JavaScript. This is not unique to Svelte. Frameworks with their own template languages, such as Vue, face the same problem.
...
Start with the end-to-end experience of a user (or an agent). I measured on Flyle's production frontend (a SvelteKit app; the official svelte-check checks 8,795 files). The effect splits into two steps.

```
svelte-check + TypeScript LS   51.4s   <- current CI configuration
      | switch to rsvelte (TypeScript stays on JS 5.9.3)
rsvelte-check + tsc            30.6s   (1.7x; CPU 104.9s -> 45.9s, about 56% less)
      | switch the check engine to tsgo
rsvelte-check + tsgo            9.0s   (5.7x combined; CPU 32.1s)
```
...
The 1.9x gap at N=1 shrinks as concurrency rises, and it nearly disappears with 8 simultaneous checks (14.1s vs 13.5s). This result suggests that **under high load where checks always overlap, the tsgo work shared by both setups becomes the bottleneck** (per-check CPU, total CPU divided by 8, is still about 21% lower at 10.8 vs 8.5 CPU-seconds, and total RSS is similar; capping rayon threads at 12/N made no significant difference). In real use, however, check start times are spread out, so I believe shorter individual runs reduce the overlap itself. This simultaneous-launch benchmark does not measure that directly; verifying it with real agent execution logs is future work.

#### 3.3.4 Svelte-specific linting

I measured linting with the same rigor (see "Measurement details" for the setup). This is not a replacement of a whole ESLint setup. It is **a comparison with the 37 shared Svelte-specific rules at identical severities, with every other rule disabled on both sides**. With the executed rule sets made identical, both tools produced **exactly the same 382 diagnostics**.
...
The median ratio across 10 interleaved pairs was **19.4x** (range 18.0-23.5). Unlike type checking, linting has no shared dependency like a TS engine, so the difference stays this large even end to end. Memory also drops sharply, from 0.86GB to 0.05GB.
...
**Official maintenance under the Svelte organization.** Nothing is decided here; this is a direction I am aiming for over the long term. A toolchain needs community trust and continuity, and a personal repository has limits. I see the compatibility-first design principle as the groundwork for one day reaching a level where maintenance under the Svelte organization can be proposed.

## Summary

- AI agents became the main implementers, and static checks became something that runs on every loop iteration. Check time is the lower bound of iteration time, and check resource usage limits parallelism
- Rust / native toolchains are solving this, but the Svelte-specific parts of `.svelte` processing still run on JavaScript
- rsvelte is a set of Rust implementations that aims to be a drop-in replacement for the Svelte toolchain. It adds nothing to the language semantics, and it puts low-cost switching and rollback first. Maturity varies by package
- On Flyle's production frontend, the rsvelte switch alone made type checking 1.7x faster (51.4s to 30.6s, about 56% less CPU), and 5.7x combined with tsgo (9.0s). Svelte-specific linting with an identical rule set is about 20x faster. rsvelte-check itself uses about 0.12GB of memory; the TS engine takes most of the rest. You can try it alongside the official tools while comparing diagnostic differences
